...
pub fn right_pad<T: Copy>(str: &mut Vec<T>, len: usize, pad: T) {
...
pub fn left_pad<T: Copy>(str: &mut Vec<T>, len: usize, pad: T) {
    let old_str = snapshot! { str };
...
#[logic]
pub fn is_unique<T>(s: Seq<T>) -> bool {
    pearlite! {
...
#[logic]
pub fn contains<T>(seq: Seq<T>, elem: T) -> bool {
    pearlite! {
...
#[logic]
pub fn is_subset<T>(sub: Seq<T>, sup: Seq<T>) -> bool {
    pearlite! {
...
pub fn subset_push<T>(s: Seq<T>, elem: T) {}
...
pub fn insert_unique<T: Eq + DeepModel>(vec: &mut Vec<T>, elem: T) {
...
pub fn unique<T: Eq + DeepModel + Copy>(str: &[T]) -> Vec<T> {
...
#[ensures(result >= 0)]
pub fn sum_range(seq: Seq<u32>, from: Int, to: Int) -> Int {
    if to - from > 0 {
...
pub fn sum_range_split(seq: Seq<u32>, from: Int, to: Int, i: Int) {
    if i > from {
...
pub fn score(seq: Seq<u32>, i: Int) -> Int {

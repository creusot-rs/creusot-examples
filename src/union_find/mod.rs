...
impl<T> UnionFind<T> {
    ...
    pub fn new() -> Self {
        Self {
            ...
            map: FMap::new(),
            values: snapshot!(conjure()),
            distance: snapshot!(conjure()),
            root_of: snapshot!(conjure()),
    ...
    #[requires(self.domain().contains(elem))]
    #[ensures(result == self.root_of()[elem])]
    #[ensures(self.unchanged())] // TODO: could we hide this mutation, and take `&self` ?
    ...
    #[ensures(self.unchanged())]
    pub fn eq(&mut self, e1: Element<T>, e2: Element<T>) -> bool {
        let r1 = self.find(e1);
        let r2 = self.find(e2);
        std::ptr::addr_eq(r1.0, r2.0)
    }

    /// `true` if `x` and `y` are in the same class.
    #[predicate]
    #[open]
    pub fn equiv(self, x: Element<T>, y: Element<T>) -> bool {
    ...
    #[ensures(forall<z: Element<T>> self.domain().contains(z) ==> (^self).root_of()[z]
        == if (*self).equiv(z, x) || (*self).equiv(z, y) {
            result
        } else {
            (*self).root_of()[z]
        }
    ...
    fn link(&mut self, x: Element<T>, y: Element<T>) -> Element<T> {
        ...
        if rx < ry {
            ...
            *PtrOwn::as_mut(x.0, perm_mut_x) = Content::Link(y);
            ...
            self.distance = snapshot!(self
                .distance
                .set(y, 1 + self.distance[x].max(self.distance[y])));
            y
        } else {
            ...
            *PtrOwn::as_mut(y.0, perm_mut_y) = Content::Link(x);
            ...
            self.distance = snapshot!(self
                .distance
                .set(x, 1 + self.distance[x].max(self.distance[y])));
            x
        }
    }
    ...
    #[ensures(exists<r: Element<T>> (r == (*self).root_of()[x] || r == (*self).root_of()[y]) && {
        forall<z: _> self.domain().contains(z) ==> {
            ((^self).root_of()[z] == if (*self).equiv(z, x) || (*self).equiv(z, y) {
                r
            } else {
                (*self).root_of()[z]
            }) && ((^self).values()[z] == if (*self).equiv(z, x) || (*self).equiv(z, y) {
                (^self).values()[r]
            } else {
                (*self).values()[z]
            })
